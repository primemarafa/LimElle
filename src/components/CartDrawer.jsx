...
export default function CartDrawer({ items, onClose, onQuantityChange, onRemove }) {
  ...
  const reference = useMemo(() => `LE-${new Date().getFullYear()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`, []);
  const message = `Bonjour Lim'Elle 🌸\nJe souhaite vérifier cette commande :\n\nRéférence : ${reference}\n${items.map((item) => `• ${item.product.name} x${item.quantity} | Taille : ${item.product.selectedSize ?? "Unique"} | Couleur : ${item.product.selectedColor ?? "Standard"}`).join("\n")}\n\nPoids estimé : ${weight.toFixed(2)} kg\nPrix global indicatif : ${formatXof(total)}\n\nMerci de confirmer la disponibilité et le prix final.`;
    ...
    <div className="fixed inset-0 z-50 bg-black/40" onClick={onClose}>
      <aside className="ml-auto flex h-full w-full max-w-md flex-col bg-[#F5F0E6] p-5" onClick={(event) => event.stopPropagation()}>
        ...
        {!items.length ? <div className="flex flex-1 flex-col items-center justify-center text-center text-[#5B5348]"><ShoppingBag size={42} /><p className="mt-4">Ton panier est vide.</p></div> : <>
          <div className="mt-6 flex-1 space-y-3 overflow-y-auto">{items.map((item) => { const key = itemKey(item.product); return <div key={key} className="rounded-2xl bg-white p-3"><div className="flex gap-3"><img src={item.product.img} alt={item.product.name} className="h-20 w-16 rounded-xl object-cover" /><div className="min-w-0 flex-1"><div className="font-bold">{item.product.name}</div><div className="mt-1 text-xs text-[#5B5348]">Taille : {item.product.selectedSize ?? "Unique"} · Couleur : {item.product.selectedColor ?? "Standard"}</div><div className="mt-1 text-sm text-[#5B5348]">{formatXof(item.product.price)}</div><div className="mt-2 flex items-center justify-between"><div className="flex items-center gap-2"><button onClick={() => onQuantityChange(key, Math.max(1, item.quantity - 1))} className="rounded-full bg-[#EBE3D2] p-1"><Minus size={14} /></button><span>{item.quantity}</span><button onClick={() => onQuantityChange(key, item.quantity + 1)} className="rounded-full bg-[#EBE3D2] p-1"><Plus size={14} /></button></div><button onClick={() => onRemove(key)} className="text-[#A6512F]"><Trash2 size={17} /></button></div></div></div></div>; })}</div>

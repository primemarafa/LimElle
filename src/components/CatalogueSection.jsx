...
export default function CatalogueSection({ categories, products, activeCategory, onCategoryChange, onProductSelect }) {
  ...
  return (
    <section id="catalogue" className="bg-[#FAF5EE] px-5 pb-20 pt-16 md:pt-24">
      <div className="mx-auto max-w-7xl">
        <div id="categories" className="scroll-mt-28 text-center">
          <p className="text-xs font-bold uppercase tracking-[.24em] text-[#96654C]">Nos catégories</p>
          <h2 className="mt-3 font-serif text-4xl tracking-[-.03em] text-[#3A2E28] md:text-5xl">Trouvez ce qui vous sublime</h2>
        </div>
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {visualCategories.map((category) => {
            const visual = CATEGORY_VISUALS[category.id];
            return (
              <button key={category.id} type="button" onClick={() => onCategoryChange(category.id)} className="group relative aspect-[.88] overflow-hidden rounded-2xl text-left shadow-sm ring-1 ring-[#3A2E28]/10">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#2A211C]/80 via-[#2A211C]/15 to-transparent" />
        ...
        <div id="products" className="mt-24 scroll-mt-28">
          <div className="flex items-end justify-between gap-4">
            <div><p className="text-xs font-bold uppercase tracking-[.22em] text-[#96654C]">Produits phares</p><h2 className="mt-3 font-serif text-4xl tracking-[-.03em] text-[#3A2E28] md:text-5xl">Nos coups de cœur</h2></div>
            <button type="button" onClick={() => onCategoryChange("all")} className="hidden rounded-lg px-2 py-2 text-sm font-bold text-[#3A2E28] hover:bg-white sm:block">Voir tout <span className="ml-2">→</span></button>
          </div>

    ...
    <section className="mx-auto max-w-5xl px-5 pb-16 pt-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          {product.sizes?.length > 0 && <div className="mt-7"><label className="text-sm font-bold">Taille</label><div className="mt-2 flex flex-wrap gap-2">{product.sizes.map((item) => <button key={item} onClick={() => setSize(item)} className={`rounded-full border px-4 py-2 text-sm font-bold ${size === item ? "border-[#C1613F] bg-[#C1613F] text-white" : "border-black/10 bg-white"}`}>{item}</button>)}</div></div>}

          {product.colors?.length > 0 && <div className="mt-6"><label className="text-sm font-bold">Couleur</label><div className="mt-2 flex flex-wrap gap-2">{product.colors.map((item) => <button key={item} onClick={() => setColor(item)} className={`rounded-full border px-4 py-2 text-sm font-bold ${color === item ? "border-[#C1613F] bg-[#C1613F] text-white" : "border-black/10 bg-white"}`}>{item}</button>)}</div></div>}

          <div className="mt-6 flex items-center justify-between rounded-2xl bg-[#EBE3D2] p-3"><span className="text-sm font-bold">Quantité</span><div className="flex items-center gap-3"><button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="rounded-full bg-white p-2"><Minus size={16} /></button><strong>{quantity}</strong><button onClick={() => setQuantity(quantity + 1)} className="rounded-full bg-white p-2"><Plus size={16} /></button></div></div>
          <WhatsAppButton message={message} className="mt-5 w-full bg-[#3FBF63] text-white">Vérifier et commander</WhatsAppButton>
        </div>
      </div>
    </section>
  );

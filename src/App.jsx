    ...
    <main className="min-h-screen bg-[#F5F0E6] text-[#2B2620]">
      <header className="sticky top-0 z-40 border-b border-black/10 bg-[#F5F0E6]/95 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
          <div><div className="text-2xl font-semibold">Lim'Elle</div><div className="text-[10px] font-bold tracking-[.14em] text-[#5B5348]">SHOPPING DAKAR → NIAMEY</div></div>
          <div className="flex items-center gap-2">
            <button onClick={() => setCartOpen(true)} className="relative rounded-full bg-white p-3" aria-label="Ouvrir le panier"><ShoppingBag size={18} />{cartCount > 0 && <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-[#C1613F] px-1 text-[10px] font-bold text-white">{cartCount}</span>}</button>
      ...
      {selectedProduct ? <ProductDetails product={selectedProduct} onBack={() => setSelectedProduct(null)} onAddToCart={addToCart} /> : <>
        ...
        <CatalogueSection categories={CATEGORIES} products={products} activeCategory={filter} onCategoryChange={setFilter} onProductSelect={setSelectedProduct} />

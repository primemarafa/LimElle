...
export function registerRoutes(app, { products, productRepository, orderRepository, db = null, orders = new Map() }) {
  ...
  app.post("/api/orders", { schema: { body: orderBodySchema }, config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (request, reply) => {
    ...
    const order = {
      reference: createReference(), lookupToken: createLookupToken(), status: "EN_ATTENTE", customer: payload.customer,
      items: normalizedItems, deliveryMode: payload.deliveryMode, deliveryAddress: payload.deliveryAddress || "",
      notes: typeof payload.notes === "string" ? payload.notes : "",
      totals: { productTotal, weight, transport, total: productTotal + transport }, createdAt: new Date().toISOString(),
    };
